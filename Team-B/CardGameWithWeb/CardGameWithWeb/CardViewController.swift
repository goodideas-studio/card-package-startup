...
class JavaScriptFunc : NSObject, JavaScriptFuncProtocol {
    func saveData(_ character: String, background: String, conis coins: String) {
        ...
        let drawInfo = ["character": character, "background": "cardBackground0\(Int(background)!+1)"]
        print(drawInfo)
    ...
    func returnCoins() -> String {
        
        return UserDefaults.standard.string(forKey: "coins")!
...
//        let context: JSContext = JSContext()
        ...
        let testContext = self.mainWebView.value(forKeyPath: "documentView.webView.mainFrame.javaScriptContext") as? JSContext
